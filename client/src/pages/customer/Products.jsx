...
function Products({ setCart }) {
  ...
  let filteredProducts = products.filter(p =>
    (p.brand.toLowerCase().includes(search.toLowerCase()) || p.name.toLowerCase().includes(search.toLowerCase())) &&
    (category === "All" || p.category === category) && (brand === "All" || p.brand === brand)
  );
    ...
    <>
      ...
      <div className="min-h-screen bg-black text-white">
        <div className="h-28 flex items-center justify-end border-b border-white/20">
          <input type="text" placeholder="Search..." value={search} onChange={e => setSearch(e.target.value)} className="w-100 h-10 mr-10 pl-4 pr-10 text-black text-md rounded-full outline-none bg-white focus:ring-2 focus:ring-gray-400" />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 max-w-7xl mx-auto">
          <aside className="border-r border-white/20">
            <div className="p-7 border-b border-white/20">
              <h2 className="text-xs text-gray-400 font-bold mb-3 tracking-widest">SORT</h2>
              <select value={sort} onChange={e => setSort(e.target.value)} className="w-full h-9 px-3 bg-black text-white text-sm border border-white/30 rounded outline-none hover:border-white transition">
            ...
            <div className="p-7">
              <h2 className="text-xs text-gray-400 font-bold mb-4 tracking-widest">FILTER</h2>
              <select value={category} onChange={e => setCategory(e.target.value)} className="w-full h-9 mb-3 px-3 bg-black text-white border border-white/30 rounded outline-none hover:border-white transition">
                {categories.map(c => <option key={c}>{c}</option>)}
              </select>
              <select value={brand} onChange={e => setBrand(e.target.value)} className="w-full h-9 px-3 bg-black text-white border border-white/30 rounded outline-none hover:border-white transition">
          ...
          <main className="col-span-3">
            ...
            {!filteredProducts.length && <p className="py-20 text-center text-gray-400">No products found.</p>}
          </main>
        </div>
      </div>
    </>
...
function ProductCard({ product, setCart }) {
  ...
  const addToCart = () => {
    setCart(prevCart => {
      ...
      if (existing) {
        return prevCart.map(item => item.id === product.id ? { ...item, quantity: Math.min(item.quantity + 1, product.stock) } : item);
      }
